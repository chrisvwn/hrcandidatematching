...
#      {
        ...
        if(is.null(remDr$client))
        {
          repeat
          {
            ...
            res <- remDr$client <- try(remoteDriver(remoteServerAddr='104.196.11.173', port=4444L, browserName="chrome", extraCapabilities = eCaps), TRUE)
        ...
        repeat
        {
          if(cancelClientSearch)
          {
            if(exists("clientId") && clientId != "")
            {
              ...
              removeClientFromUserQueue(userId, siteId, clientId)
          ...
          for(searchTxt in searchTxts)
          {
            ...
            repeat
            {
              message(Sys.time(), ": Find the order link")
              orderDiv <- suppressMessages(try(remDr$client$findElement('class', "no-wrap"),TRUE))
              
              orderLink <- suppressMessages(try(orderDiv$findChildElement('css','a'),TRUE))
              
              if(class(orderLink) == "try-error")
              {
                repeatCount <- repeatCount + 1
            ...
            message(Sys.time(), ": Found. Clicking sort link")
            res <- try(orderLink$clickElement(), TRUE)
            ...
            repeat
            {
              ...
              if(length(jobNodes) > 0)
              {
                ...
                message(Sys.time(), ": Getting job dates")
                jobDataAnuncios <- unlist(sapply(jobNodes, 
                                              function(x)
                                              {
                                                a <- suppressMessages(try(x$findChildElement("class", "date"), TRUE))
                                                
                                                if(class(a) == 'try-error')
                                                  dataAnuncio <- NA
                                                else
                                                  dataAnuncio <- a$getElementText()
                                                
                                                if(grepl("hores|hoje|recém", dataAnuncio, ignore.case = T))
                                                   dataAnuncio <- Sys.Date()
                                                else if(grepl("\\d+ dias", dataAnuncio, ignore.case = T))
                                                {
                                                  daysPast <- stringr::str_extract(dataAnuncio, '\\d+')
                                                  dataAnuncio <- lubridate::as_date(Sys.Date()) - lubridate::period(paste0(daysPast, " days"))
                                                } else
                                                {
                                                  dataAnuncio <- Sys.Date()
                                                }
                ...
                message(Sys.time(), ": Getting job Descriptions")
                jobDescriptions <- unlist(sapply(1:length(jobNodes),
                                              function(i)
                                              {
                                                jobDetLinkElem <- jobNodes[[i]]$findChildElement('class','turnstileLink')

                                                jobDetLink <- unlist(jobDetLinkElem$getElementAttribute('href'))
                                                
                                                message("Opening job description: ", jobDetLink)
                                                
                                                remDr$client$navigate(jobDetLink)

                                                message("Reading job description")
                                                jobDescElem <- try(remDr$client$findElement('class','jobsearch-JobComponent-description'),TRUE)
                                                
                                                if(class(jobDescElem) == 'try-error')
                                                  jobDescElem <- try(remDr$client$findElement('id','job_summary'),TRUE)
                                                
                                                if(class(jobDescElem) == 'try-error')
                                                  return(NA)

                                                jobDesc <- jobDescElem$getElementText()
                                                
                                                message("Going back to the search results page")
                                                
                                                remDr$client$goBack()
                                                
                                                Sys.sleep(3)
                                                
                                                message("Re-reading job Nodes")
                ...
                message(Sys.time(), ": Combining job details")
                #cbind jobnames with details
                jobAds <- dplyr::bind_cols("JobTitle"=jobTitles, "JobUrl"=jobUrls, "CompanyName"=jobCompanies, "AnnounceDate"=jobDataAnuncios, "DescriptionShort"=jobSummaries, "DescriptionLong"=jobDescriptions)
          ...
          removeClientFromProcessing(userId, siteId, clientId)
          removeClientFromQueue(userId, siteId, clientId)
